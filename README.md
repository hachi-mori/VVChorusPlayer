# VVChorusPlayer

## ✨ これは何？

**VOICEVOX プロジェクトファイル（`.vvproj`）を直接読み込み、
複数キャラの合唱音声を生成・再生できる VST3 プラグイン／スタンドアロンアプリ** です。

歌唱キャラの自動編成（オート/ランダム/パート別）に対応し、合唱向けのキャラ選定と歌声生成を素早く行えます。

## 🎯 できること

- `.vvproj` の読み込み（songトラック）
- `.vvproj` のトラック選択（songトラック名から選択）
- 複数キャラによる合唱生成
- 手動選択モード
- 自動編成モード（`オート` / `ランダム` / `パート別`）
- パート別選択（`女声` / `男声` / `ソプラノ` / `アルト` / `テノール` / `バス`）
- 歌唱キャラ数の指定
- キャラの音域調整量に基づく合成
- 楽譜音域に応じた動的オクターブ補正
- プレビュー再生 / 停止
- WAV書き出し / 歌唱キャラクレジット出力

## ⚙ 動作に必要なもの

- ソングトラックに楽譜情報が入った VOICEVOX プロジェクトファイル（`.vvproj`）
- 本プラグイン（VST3）／本アプリ（スタンドアロン）
  - リリース: https://github.com/hachi-mori/VVChorusPlayer/releases
- VST3を使用する場合は VST3に対応しているDAW
- VOICEVOX を起動していること
  - https://voicevox.hiroshiba.jp/

## 🚀 使い方（クイックスタート）

<img width="1728" height="988" alt="image" src="https://github.com/user-attachments/assets/8932dfd3-04dc-44ab-b475-db639746e98d" />

## ① VOICEVOX を起動

VOICEVOX（または VOICEVOX Engine）をローカルで起動します。

## ② アプリ / プラグインを開く

アプリ（.exe）を起動します。

あるいは、DAW でVST3 `VVChorusPlayer` トラックを挿入します。

## ③ `.vvproj` を選択

`vvprojを選択` ボタンから対象ファイルを読み込みます。
読み込み後、右側のトラックリストから生成対象トラックを選択します。

## ④ 歌唱キャラクターを選ぶ

`手動選択` または `自動編成` を選びます。

- 手動選択
  - キャラ一覧から任意のキャラ数をチェックして選択
  - `表示` トグルで `全スタイル表示 / 先頭のみ` を切り替え可能
- 自動編成
  - `編成方式` を選択（`オート` / `ランダム` / `パート別`）
  - `パート別` 選択時は `パート` を選択
  - `歌唱人数` を指定

## ⑤ 歌声を生成

`生成スタート` を押して合唱音声を生成します。
生成ログ欄に進捗が表示されます。

## ⑥ プレビュー再生

`再生` / `停止` で確認できます。
下部に再生位置スライダーと波形が表示されます。

## ⑦ WAV 書き出し

`生成した歌声をWAVで書き出し` からファイル保存します。

`歌唱キャラクター名を出力` からクレジットをコピーあるいは保存します。


## 🔎 現在の仕様メモ

- 自動編成では先頭スタイル（ノーマル歌唱）を使用します
- 自動でパン配置の調整を行います
- 音域調整量は `Source/keyshift_table.json` を参照します
- 同一音域調整量が重なりすぎないよう、必要に応じて軽微なオフセットを加えます
- 入力楽譜の平均音高とキャラ音域を照合し、必要に応じて動的オクターブ補正を行います

---

## 技術構成

- **Language**: C++
- **Framework**: JUCE
- **Format**: VST3
- **External**: VOICEVOX Engine API（HTTP）
